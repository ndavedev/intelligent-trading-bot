...
def main(config_file):
    ...
    if file_path.suffix == ".parquet":
        ...
        df = cudf.read_csv(file_path, parse_dates=[time_column], nrows=P.in_nrows)
    else:
    ...
    df = df.tail(P.tail_rows) if P.tail_rows > 0 else df
    ...
    for label in labels:
        if df[label].dtype == 'bool':
            df[label] = df[label].astype('int32')  # For classification tasks we want to use integers
    ...
    if label_horizon:
        df = df.head(len(df) - label_horizon)

    df.replace([cp.inf, -cp.inf], cp.nan, inplace=True)
    if len(df) == 0:
    ...
    out_df = cudf.DataFrame()  # Collect predictions
    models = dict()
    scores = dict()
    ...
    for i, fs in enumerate(train_feature_sets):
        ...
        out_df = cudf.concat([out_df, fs_out_df], axis=1)
